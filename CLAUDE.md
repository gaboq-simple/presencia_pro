...
| completed_at | timestamptz nullable | Instante REAL del cierre. Es la ATRIBUCIÓN del dinero: una cita de ayer cobrada hoy es de hoy (D6). **Lo sella el trigger `seal_appointment_charge` si el escritor no lo puso** (S9-DIN-02): de los tres escritores de `completed` sólo uno lo escribía, y una cita cerrada sin él se evapora del cuadre |
| charged_by_staff_id | uuid nullable FK → staff.id | **Quién CERRÓ el cobro**, sellado en la transición a `completed` y **nunca reescrito** (freeze-once, molde de `seal_appointment_price`). Es distinto de `modified_by_staff_id`, que es la última persona que TOCÓ la fila y la pisa cualquier edición posterior — con 5 barberos cobrando en su silla, esa diferencia es la custodia del efectivo. Base de la raya (liquidación por barbero); su consumidor todavía no existe, igual que `staff.compensation_model`. Sin backfill: las citas viejas quedan NULL, porque copiar `modified_by_staff_id` hacia atrás fabricaría la atribución que la columna vino a tener de verdad |
