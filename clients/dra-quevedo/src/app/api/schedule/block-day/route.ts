// ─── API Route: POST /api/schedule/block-day ───────────────────────────────────
// Called by the doctor from the dashboard to block a full day.
...
import { z } from 'zod';
import { blockDay } from '@presenciapro/engine/scheduling';
...
export async function POST(request: Request): Promise<Response> {
  ...
  // Guard: active doctor session (anon client reads the Authorization header)
  ...
  const { date: dateStr, specialistId, reason } = parsed.data;
  ...
  // Return the canonical local date string (YYYY-MM-DD) for the UI to use
  const localDate = new Intl.DateTimeFormat('en-CA', {
    timeZone: clientConfig.client.timezone,
    year: 'numeric', month: '2-digit', day: '2-digit',
  }).format(date);

  return json({ blocked: true, date: localDate }, 200);
}
