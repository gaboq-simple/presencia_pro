...
- **MT-02 · Login por PIN acotado al negocio correcto** 🟢 done (2026-07-07)
  `api/auth/pin/route.ts` hacía `SELECT … FROM staff WHERE pin=$1 AND active=true LIMIT 1` sin `business_id`. El PIN es único **por negocio** (`UNIQUE(business_id,pin)`), no global → con N clientes, dos barberos pueden compartir PIN y `.limit(1)` metía al barbero **al dashboard del negocio equivocado**. Fix (B1, sin migración): ruta nueva `/[slug]/staff` resuelve el negocio desde el slug y scopea el query con `.eq('business_id', …)`; `/staff` pelado ahora muestra un prompt "¿cuál es tu barbería?" (sin listar negocios) que rutea a `/[slug]/staff`; slug desconocido → 401/`notFound()`, nunca fallback que adivine. Verificado por ruta real (seed de 2º negocio + barbero PIN colisionante 1234): pre-fix el query sin scope matcheaba 2 filas en 2 negocios; post-fix cada slug resuelve a su negocio correcto y el slug inválido no cae en ninguno. Nota: el dev-server de preview corre el checkout `main` (launch script hardcodea esa ruta), así que la prueba HTTP no ejercita el worktree; el scope se verificó determinísticamente contra prod replicando las queries del handler + tsc/build prueban el cableado (`ƒ /[slug]/staff`).
