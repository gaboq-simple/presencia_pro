...
| 2026-06-26 | Hallazgo 4 — "cualquier día"/"otro día" no se entendían (no-preferencia de FECHA, gemelo R4.2) | done-pending-smoke (PR abierto) | Gemelo EXACTO de R4.2 (no-preferencia de barbero) en el eje **FECHA**: el smoke (Img 4) mostró que **"cualquier día"** → "No entendí bien qué día prefieres" y **"otro día"** tras ver slots → mismo rechazo. **Eje resuelto:** `interpretation.noPreference` es **axis-agnostic** (CRUDO, solo presencia de keyword); **el ESTADO fija el significado** — en `qualifyingDatetime` "cualquiera" = FECHA porque el barbero YA se resolvió en QUALIFYING_STAFF (en `qualifyingStaff` el mismo flag = barbero). **Fix 1 ("cualquier día", `qualifyingDatetime`):** chequeo `noPreference` **ANTES del clasificador** (determinista, evita Haiku; patrón R4.1 con fallback estrangulador `interp ? interp.noPreference : NO_PREFERENCE_KEYWORDS.some(...)`). **Opción A:** setea `requestedDate=HOY` y **delega a SHOWING_SLOTS** (chequea hoy, cae a `findSlotsInNextDays` si vacío) = primer día con cupo — **NO** `findSlotsInNextDays` directo (arranca en hoy+1 → saltearía un hoy con cupo). **Orden "fecha concreta gana":** el bloque `parseDate` corre antes → "cualquier día… el martes" respeta el martes. **Fix 2 ("otro día", `confirmingAppointment`):** **interceptación en el handler ANTES de `routeSlotSelection`** (el **router puro queda INTACTO** — `slotSelection.test.ts:188` "otro día"→date_redirect sigue verde). Hoy `date_redirect` **borra `requestedDate`** (pierde el ancla) y rebota a QUALIFYING_DATETIME, que no sabe parsear "otro día" → "no entendí". El handler ancla `requestedDate = día mostrado + 1` → SHOWING_SLOTS (muestra ese día o cae a `findSlotsInNextDays`). Detector `detectsNextDayRedirect` **EXIGE `parseDate==null`** (mismo probe sin frases de turno que el router): **caveat (3)** "otro día, el martes" lleva fecha concreta → `false` → cae al router → date_redirect (parsea el martes). Blindado en **2 niveles** (detector puro + handler). **2 notas de comportamiento (aprobadas):** "otro día" **limpia el turno** (muestra el día completo, re-filtrable); el cambio de barbero cuando día+1 está vacío es **pre-existente** (zero-slot path). Circularidad `qualifyingDatetime↔interpreter` (nuevo import `NO_PREFERENCE_KEYWORDS`) **compila limpia** (binding call-time, como confirming/staff). Tests: +2 `interpreter.test.ts` ("cualquier día"=noPreference+date null; "otro día"=¬noPreference+date null), **nuevo `tests/dateNoPreference.test.ts`** (+7: handler "cualquier día" con/sin barbero + fecha-gana; detector puro; handler "otro día"→ancla+1; caveat 3 en 2 niveles). **Malla 456/456 verde**, app `tsc` 0. Rama `fix/date-no-preference` (doc del hallazgo en `c5bd966`, 1er commit de la rama). PR contra main; **smoke pendiente** (lo hace Gabriel). |
---
