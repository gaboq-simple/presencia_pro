...
- **PR-C · Clientela: movimiento entre grupos** ⚪ todo
  Flujo mes-a-mes (Nuevos→Regulares, Regulares→Se-están-yendo). **NO requiere snapshots** (no hay tabla de histórico de segmento y NO hace falta): el segmento es `computeCadence(visitas ≤ T, T)` determinista → se reconstruye recomputando a dos `now` sobre la historia inmutable. Trivial mientras la historia es rala. Sin schema.
