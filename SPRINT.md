...
#### S6-OPS-01 — Arreglar backup automático a R2 (nunca funcionó) 🔵 in-progress (2026-06-29, rama `feat/fix-r2-backup`)
**Origen:** descubierto durante Fase 2a (sistema de control), al pedir Gabriel confirmar punto de retorno antes de un DDL en prod. El backup semanal a R2 (`backup-weekly.yml`, entregado en **S1-OPS-01**) **nunca produjo un backup**: 0 corridas exitosas en todo el historial; las últimas 4 (07/14/21/28-jun) fallan en <30s. Gabriel opera prod (clientes reales + bot WhatsApp) **sin red de seguridad**. Bloquea aplicar la migración 044 de Fase 2a hasta tener un backup real.

**Diagnóstico (read-only, anclado en el CLI 2.75 y el historial de Actions):** tres fallas encadenadas —
1. `supabase db dump --project-ref … --output …`: en el CLI 2.x **`--project-ref` y `--output` no existen** en `db dump` (la ruta de archivo es `-f`) → imprime help y sale 1 (el "Help requested" del log).
2. **Sin credencial de DB**: `db dump` se conecta a Postgres y necesita `--db-url`/password; los secrets configurados (ACCESS_TOKEN + R2_* + passphrase) no incluyen ninguno.
3. **Proyecto equivocado**: el workflow hardcodea `uhhatetytaucucihfyyy`; el prod en vivo es `hdqazbuxtpavtioufrsv` (confirmado vía MCP).
   Bonus latente: un `db dump` sin flags trae **solo el esquema** (sin datos).

**Fix (rama `feat/fix-r2-backup`):** `scripts/backup-supabase.sh` reescrito — `--db-url` + `-f`, dump de **3 partes** (roles best-effort + esquema + datos `--data-only --use-copy`), asserts de no-vacío/no-truncado (incluye grep de `COPY`/`INSERT` en los datos), y **verificación post-upload en R2** (`head-object`, tamaño>0 — no confía en que `cp` no falló). Workflow: env `SUPABASE_DB_URL` en vez del ref equivocado. README actualizado. `bash -n` verde.

**🔴 BLOQUEADO en acción humana:** Gabriel debe agregar el secret `SUPABASE_DB_URL` (connection string **direct** o **session pooler** del proyecto `hdqazbuxtpavtioufrsv`, password percent-encoded; NO transaction pooler) en GitHub → Settings → Secrets. Sin ese secret no se puede verificar.

**Criterios de aceptación:**
- [ ] Secret `SUPABASE_DB_URL` agregado (Gabriel).
- [ ] `workflow_dispatch` corre verde y el script reporta el objeto verificado en R2 (tamaño>0).
- [ ] El dump trae **datos** (no solo esquema) — el assert de `COPY`/`INSERT` lo garantiza.
- [ ] El objeto existe en R2 (evidencia: nombre + tamaño).

**Alcance:** solo que el backup PRODUZCA un backup real y verificable. **NO** el restore drill completo (S4-OPS-02, sigue ⚪ todo). **NO** mezclar con Fase 2a (que queda en pausa hasta tener backup + aplicar 044). Nota: S1-OPS-01 queda marcada done pero su entregable de backup era no-funcional — esta tarea lo corrige.
...
| 2026-06-29 | S6-OPS-01 — Arreglar backup R2 (nunca funcionó) | in-progress (fix listo, bloqueado en secret) | Descubierto en Fase 2a al confirmar punto de retorno antes del DDL de la 044. El backup semanal a R2 (`backup-weekly.yml`, S1-OPS-01) **nunca produjo un backup**: 0 corridas exitosas, últimas 4 fallan en <30s. Tres fallas: (1) `--project-ref`/`--output` no existen en `supabase db dump` 2.x → "Help requested" exit 1; (2) sin credencial de DB (solo ACCESS_TOKEN+R2_*+passphrase); (3) ref equivocado `uhhatetytaucucihfyyy` vs prod real `hdqazbuxtpavtioufrsv` (vía MCP); + bonus: dump sin flags trae solo esquema sin datos. **Fix** (`feat/fix-r2-backup`): script reescrito con `--db-url`+`-f`, dump 3-partes (roles best-effort+esquema+datos `--data-only --use-copy`), asserts de no-vacío + grep `COPY`/`INSERT`, y verificación post-upload en R2 (`head-object` tamaño>0). Workflow usa `SUPABASE_DB_URL`. README actualizado. `bash -n` verde. **🔴 BLOQUEADO:** Gabriel agrega secret `SUPABASE_DB_URL` (direct/session-pooler, password encoded) → luego `workflow_dispatch` + verificar objeto en R2. NO es el restore drill (S4-OPS-02). NADA aplicado a prod. |
---
