...
- **AUD-07b 🟠 — Error técnico ≠ "no te entendí" ⚪ todo.** Timeout/429/5xx del LLM producen el mismo `fallbackMessage` que una incomprensión real, consumen `fallbackAttempts` y encadenan a escalación. Distinguir: mensaje de "ando lento, inténtalo de nuevo" sin gastar intentos; `failure_reason` ('timeout'|'parse'|'api') en el metadata de bot_logs; propagar `businessId`/phone a los clasificadores (hoy `''` — 429s anónimos en multi-tenant). Desbloquea el diagnóstico de todo lo demás.
