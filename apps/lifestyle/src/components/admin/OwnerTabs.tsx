...
export default function OwnerTabs({
  ...
  const [active, setActive] = useState<TabKey>('panorama');
      ...
      >
        <ul className="mx-auto flex max-w-2xl">
          {TABS.map((t) => {
            ...
            // Actividad (forense) relegada: separador a la izquierda + nunca se enciende
            // en teal (gris aun activa) → presente pero sin competir con las 3 primarias.
            const relegated = t.key === 'actividad';
            const color = relegated
              ? (isActive ? 'text-ink-2' : 'text-faint')
              : (isActive ? 'text-teal-ink' : 'text-faint');
            return (
              <li key={t.key} className={`flex-1 ${relegated ? 'border-l border-line' : ''}`}>
                <button
                  type="button"
